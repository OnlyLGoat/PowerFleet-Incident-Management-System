...
export default function AssignTechnicianPanel({
  ...
  const handleAssign = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = assignmentNote.trim() || `Assigned technician ${chosenTech?.name || ""}`;
      ...
      setError("Failed to assign technician. Ensure they are active and available.");
    } finally {
      setAssigning(false);
    }
  };
      ...
      {loading ? (
        ...
        <form onSubmit={handleAssign} className="space-y-3">
          ...
          <div>
            <input
              type="text"
              placeholder="Optional dispatch instruction / note..."
